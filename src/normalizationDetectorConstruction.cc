...
normalizationDetectorConstruction::normalizationDetectorConstruction()
...
G4VPhysicalVolume* normalizationDetectorConstruction::Construct()
{
...
//   pFakeAirBack_z = pAirBack_z - (fAirBack_z/2.0 + fFakeAirBack_z/2.0);
  ...
  // G4double FAST_Energy[1] = {2.952 *eV};
  // G4double FAST_COMPONENT[1] = {1};
  ...
  // G4double Rayleigh[NUMENTRIES_2]       ={ 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm, 138.*mm}; //{1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m,1*m};
  ...
  MPT_LYSO->AddConstProperty ("SCINTILLATIONYIELD", 0 / MeV);
  MPT_LYSO->AddConstProperty ("RESOLUTIONSCALE", 1);
  ...
  G4double vikuiti_energy_im[NUMvikuiti] = 
  {
    1.25*eV, 1.31*eV, 1.34*eV, 1.38*eV, 1.42*eV,
    1.46*eV, 1.51*eV, 1.60*eV, 1.66*eV, 1.75*eV,
    1.80*eV, 1.90*eV, 1.95*eV, 2.05*eV, 2.10*eV,
    2.20*eV, 2.26*eV, 2.35*eV, 2.45*eV, 2.51*eV,
    2.71*eV, 2.81*eV, 3.01*eV, 3.21*eV, 3.41*eV,
    3.55*eV, 3.71*eV, 3.91*eV, 4.14*eV
...
//   };
  ...
  G4double vikuiti_RIndex_im[NUMvikuiti] = 
  {
    9.49, 8.88, 8.49, 8.30, 8.18, 8.22, 8.31,
        8.60, 8.62, 8.39, 8.21, 7.82, 7.65, 7.31,
        7.15, 6.85, 6.69, 6.42, 6.15, 6.03, 5.58,
        5.38, 5.02, 4.71, 4.43, 4.24, 4.06, 3.84, 3.61  
  };
  G4double vikuiti_energy_re[NUMvikuiti] = 
  {
    1.24*eV, 1.27*eV, 1.31*eV, 1.34*eV, 1.38*eV,
    1.42*eV, 1.46*eV, 4.51*eV, 1.55*eV, 1.60*eV,
    1.66*eV, 1.71*eV, 1.77*eV, 1.84*eV, 1.91*eV,
    1.99*eV, 2.07*eV, 2.16*eV, 2.26*eV, 2.37*eV,
    2.48*eV, 2.62*eV, 2.76*eV, 2.92*eV, 3.11*eV,
    3.31*eV, 3.55*eV, 3.82*eV, 4.14*eV
  };
...
//   };
  G4double vikuiti_RIndex_re[NUMvikuiti] = 
  {
    ...
    0.1, 0.6, 1.2, 0.4, 0.23, 0.17, 0.26, 1.3,
    0.24, 0.11, 0.19, 0.24, 0.25, 0.87, 0.595,
    0.18, 0.178, 0.6, 8.0, 8.0, 8.0, 8.0
  };
  const G4int NUMu = 2;
...
//   G4double reflectivityi[NUMu] = {0.0, 0.00};
  ...
  G4double FixedTransmittance_surf[NUMu] = {0, 0};
...
//     crystalReal_surf->AddConstProperty("SURFACEROUGHNESS", realsurfaceroughness); 
  ...
  G4double siliconAbsLenghtValue[]  = {0*mm,0*mm,0*mm};
...
//   G4double heads_x = 160.0*mm;
//   G4double heads_y = 180.0*mm;
//   G4double heads_z = 20.0*mm;
...
//   G4double heads_distance = 200*mm;
  ...
  G4double fCrystal_x = 2.0 * mm;
  G4double fCrystal_y = 2.0 * mm;
  G4double fCrystal_z = 20.0 * mm;
  
  G4double xCryGap = 0.6 *mm;
  G4double yCryGap = 0.6 *mm;
  
//   G4double crystalPitchXperSUB = 2.3 * mm;
...
//  phantom_log->SetVisAttributes(phantomVisulizationAttribute); // we also set here the visualization colors
  ...
  //spherical phantom
  G4Orb* phantom_box = new G4Orb("phantom",10*mm);
  G4LogicalVolume* phantom_log = new G4LogicalVolume(phantom_box,airThinLayer,"phantom",0,0,0);
  G4VPhysicalVolume* phantom_phys = new 
  ...
  G4Box*** crystal_box_dh0					= new G4Box** [nCrystalsX];
  for(int i = 0 ; i < nCrystalsX ; i++) crystal_box_dh0[i]		= new G4Box* [nCrystalsY];
  G4LogicalVolume*** crystal_log_dh0				= new G4LogicalVolume** [nCrystalsX];
  for(int i = 0 ; i < nCrystalsX ; i++) crystal_log_dh0[i]		= new G4LogicalVolume* [nCrystalsY];
  G4VPhysicalVolume*** crystal_phys_dh0 			= new G4VPhysicalVolume** [nCrystalsX];
  for(int i = 0 ; i < nCrystalsX ; i++) crystal_phys_dh0[i]		= new G4VPhysicalVolume* [nCrystalsY];
...
//   0,0  1,0  2,0  3,0 ... 
  ...
  // so the apd index is [ i % 8 , j % 12 ]

//   G4double xAPD = 11.20*mm;
//   G4double yAPD = 19.5*mm;
//   G4double xAPDgap = 1.6*mm;
//   G4double yAPDgap = 2.285714286*mm;
//   const G4int nXapd = 12;
//   const G4int nYapd = 8;
//   
//   G4double xFrameAPD = 0.5*mm;
//   G4double yFrameAPD = ;
  
//   G4double xAPDpos[nXapd];
//   for(int i = 0 ; i < nXapd ; i ++)
//   {
//     xAPDpos[i] = (0.5*xAPDgap + 0.5*xAPD)*((i*2)+1) ;
//   }
//   
//   G4double yAPDpos[nYapd];
//   for(int i = 0 ; i < nYapd ; i ++)
//   {
...
//       G4cout << name.str() << " Position" << "\t" << i*fAirThinLayerBox_x - matrixShiftX  << "\t" << j*fAirThinLayerBox_y - matrixShiftY << "\t" << matrixShiftZ << G4endl;
      ...
      name << "Crystal_" << i << "_" << j;
      //box volumes
      crystal_box_dh0[i][j]  = new G4Box(name.str().c_str(),fCrystal_x/2.0,fCrystal_y/2.0,fCrystal_z/2.0);
      //logical volumes
      crystal_log_dh0[i][j]  = new G4LogicalVolume(crystal_box_dh0[i][j],LYSO,name.str().c_str(),0,0,0);
      G4VisAttributes* CryVisulizationAttribute = new G4VisAttributes(G4Colour(0.0,1.0,1.0)); //cyan
      crystal_log_dh0[i][j]->SetVisAttributes(CryVisulizationAttribute); // we also set here the visualization colors
      // and we place the box in space 
...
//       crystal_phys_dh0[i][j] = new G4PVPlacement(0,G4ThreeVector((fCrystal_x  + xCryGap)/2.0 + (i) * (fCrystal_x  + xCryGap) - (headx/2.0)  , (fCrystal_y  + yCryGap)/2.0 +(j) * (fCrystal_y  + yCryGap) - (heady/2.0) ,0.),crystal_log_dh0[i][j],name.str().c_str(),DH0_log,false,fCheckOverlaps);
  ...
  for(int i = 0 ; i < nCrystalsX ; i++) crystal_log_dh1[i]		= new G4LogicalVolume* [nCrystalsY];
  G4VPhysicalVolume*** crystal_phys_dh1 			= new G4VPhysicalVolume** [nCrystalsX];
  for(int i = 0 ; i < nCrystalsX ; i++) crystal_phys_dh1[i]		= new G4VPhysicalVolume* [nCrystalsY];
...
//       G4cout << name.str() << " Position" << "\t" << i*fAirThinLayerBox_x - matrixShiftX  << "\t" << j*fAirThinLayerBox_y - matrixShiftY << "\t" << matrixShiftZ << G4endl;
      ...
      name << "Crystal_" << i << "_" << j;
      //box volumes
      crystal_box_dh1[i][j]  = new G4Box(name.str().c_str(),fCrystal_x/2.0,fCrystal_y/2.0,fCrystal_z/2.0);
      //logical volumes
      crystal_log_dh1[i][j]  = new G4LogicalVolume(crystal_box_dh1[i][j],LYSO,name.str().c_str(),0,0,0);
      G4VisAttributes* CryVisulizationAttribute = new G4VisAttributes(G4Colour(0.0,1.0,1.0)); //cyan
      crystal_log_dh1[i][j]->SetVisAttributes(CryVisulizationAttribute); // we also set here the visualization colors
      // and we place the box in space 
      // it is assigned to its mother volume, the air thin layer, so it just has to be placed in the middle of it
      //for the physical volumes, we use as name just a number (makes analysis easier)
      name.str("");
